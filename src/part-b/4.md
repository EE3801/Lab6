...
## 16.
Before you generate your plots, you should update the `PyHipp` and `DataProcessingTools` repositories on your computer using the command line or GitHub Desktop. Since you should have already merged the upstream changes into your repository, you can just pull down the changes from your repository.
...
## 17.
We will first load the Unity object in **ipython** on your computer by launching with the "pylab" argument, which opens figures in non-blocking mode:
...
> <p class="note">Note
> 
> Windows users should run ipython in WSL.
...
The output above shows the "In[ ]:" prompt returning after the PanGUI command, which is what the "--pylab" argument enables. If you do not see the prompt returning, close the PanGUI window, call the "create_window" function a second time, and the prompt should return, enabling non-blocking mode.
